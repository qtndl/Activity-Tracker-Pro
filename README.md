...
├── run_local.sh          # Скрипт для локального запуска бота и веб-сервера (без Docker)
├── stop_local.sh         # Скрипт для остановки локально запущенных сервисов
├── Dockerfile            # Dockerfile для сборки образа приложения
├── docker-compose.yml    # Docker Compose для развертывания с Nginx (production)
├── docker-compose.dev.yml # Docker Compose для разработки (без Nginx, с hot-reload)
...
- **Развертывание**: Docker, Docker Compose, Nginx (опционально, для production)
...
Для удобного локального запуска используйте скрипты:
...
#### Для разработки (с hot-reload, без Nginx):

```bash
docker-compose -f docker-compose.dev.yml up --build
```
Сервер будет доступен по `http://localhost:WEB_PORT` (порт из вашего `.env`).

#### Для Production (с Nginx, рекомендуется):

1.  **Настройка Nginx**:
    - Отредактируйте `nginx.conf` при необходимости (например, если используете свои SSL сертификаты).
    - Если используете SSL, поместите сертификаты в папку `certs/` (`fullchain.pem`, `privkey.pem`). Скрипт `run_local.sh` может помочь сгенерировать самоподписанные сертификаты для локального тестирования HTTPS, но для production используйте настоящие сертификаты.
2.  **Запуск**:
    ```bash
    docker-compose -f docker-compose.yml up -d --build
    ```
    Веб-интерфейс будет доступен через Nginx, обычно на порту 80 (HTTP) и/или 443 (HTTPS).
